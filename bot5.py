...
parser.add_argument("--leverage", type=int, default=10, help="Leverage multiplier (e.g., 10)")
...
for i in range(pitchfork_window, len(df)-2):
    ...
    usd_risk = balance * risk_per_trade
    notional_position = usd_risk * leverage
    units = notional_position / current_close
    ...
    if prev_close <= upper and current_close > upper:
        ...
        if next_high >= tp:
            profit = (tp - entry_price) * units
        elif next_low <= sl:
            profit = (sl - entry_price) * units
        else:
            continue
    ...
    # Breakout Down
        ...
        if next_low <= tp:
            profit = (entry_price - tp) * units
        elif next_high >= sl:
            profit = (entry_price - sl) * units
        else:
            continue
